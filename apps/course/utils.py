...
def generate_certificate(user_course, full_name):
    ...
    certificate_obj = Certificate.objects.create(course=user_course.course, user=user_course.user, full_name=full_name)
    if not certificate_obj.full_name:
        full_name = user_course.user.full_name

    template = get_template(user_course.course.certificate_html_template.path)
    context = {
        "full_name": full_name,
        "course_title": user_course.course.title,
        "author": user_course.course.author,
        "author_signature": user_course.course.author_signature.path,
    ...
    #     name=f"{user_course.course.title}_{full_name}_{certificate_obj.cid}.pdf", content=certificate
    # )
    ...
    certificate_obj.image.save(
        name=f"{user_course.course.title}_{full_name}_{certificate_obj.cid}.JPEG", content=File(output_image)
    )
